(() => {
	var entry_compare = (a, b) => {
			//each entry = [value,separator,star] where separator is mountain
			if (a[0] < b[0]) return -1;
			if (a[0] > b[0]) return 1;
						...
						column
							.map((entry) => {
								...
								if (sep.every((column) => !column.length))
									return ','.repeat(sep.length) + star + v;
								if (
									mountain_display(sep.slice(0, 2)) ===
										mountain_display([[], [[1, [[]]]]]) &&
		...
		subtract1 = (A0, V) => {
			...
			if (mountain_is_one(topright_separator)) A[rightmost].pop();
				...
				var BRseparator = S(A, BRij[0], BRij[1] - 1),
					J =
						mountain_compare(BRseparator, topright_separator) >= 0
							? [[]]
							: BRseparator.concat([[]]);
				if (
					vertical_compare(
						vertical_increase(V[BRij[0]][BRij[1] - 1] ?? [], J),
		...
		extend = (A0) => {
			...
			for (i = 0; i < top_separators.length; ++i) {
				...
				for (j = refs[i], stretch_value[i] = 0; j - 1 > refs[i - 1]; --j) {
					...
					if (mountain_compare(M, stretch_threshold[i]) < 0)
						A[rightmost][j - 1] && (A[rightmost][j - 1][2] = true);
