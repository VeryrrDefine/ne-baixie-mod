(() => {
	var entry_compare = (a, b) => {
		...
		subtract1 = (A0, V) => {
			...
			if (mountain_is_one(topright_separator)) A[rightmost].pop();
				...
				if (mountain_compare(BRseparator, topright_separator) < 0) {
					while (mountain_compare(BRseparator, (J = expand(topright_separator, i))) >= 0)
						++i;
				} else {
				...
				if (
					...
					A[rightmost].pop();
				else A[rightmost][topmost][1] = J;
			} else {
				topright_separator = topright_separator.slice(0, -1);
				if (
					vertical_compare(
						vertical_increase(V[BRij[0]][BRij[1] - 1] ?? [], topright_separator),
						V[rightmost][topmost - 1] ?? [],
					) <= 0
				)
					A[rightmost].pop();
		...
		extend = (A0, B0) => {
			var datakey = B0
				? mountain_display(A0) + '"' + mountain_display(B0)
				: mountain_display(A0);
			...
			for (i = 0; i < top_separators.length; ++i) {
				...
				if (mountain_compare(BR_separators[i], top_separators[i]) < 0) {
					while (mountain_compare(BR_separators[i], expand(top_separators[i], k0)) >= 0)
						++k0;
				}
				...
				if (
					mountain_compare(top_separators[i], ref_separators[i]) > 0 &&
					mountain_compare(ref_separators[i], stretch_threshold[i]) >= 0
				) {
					var k = k0;
					while (mountain_compare(ref_separators[i], expand(top_separators[i], k)) >= 0)
						++k;
