...
(() => {
	var data = {},
		...
		vertical_increase = (y, d) => {
			//go from y=[r0,r1,r2,...,r(d-1),rd #] to [0,0,0,...,0,rd+1 #]
			var c = y.slice();
			...
			mountain.forEach((column) => {
				...
				while (true) {
					...
					for (parent = entry; true; ) {
						...
						while (up.rightleg_up && vertical_compare(up.rightleg_up.y, parent.y) <= 0)
							up = up.rightleg_up;
						parent = up;
		...
		collect = (working_entry) =>
			...
			dimension_difference(working_entry.y, working_entry.rightleg_down.y) === 0
				? collect1D(working_entry)
				: collect_usual(working_entry),
		fill_magma_edge = (mountain, source_entry, leftleg_entry) => {
			var newentry,
				d,
				targetx = source_entry.x - source_entry.leftleg_down.x + leftleg_entry.x;
			for (
				d = dimension_difference(leftleg_entry.y, leftleg_entry.rightleg_up.y);
				d >= 0;
				--d
			) {
				newentry = {
		...
		copy_single_edge = (mountain, source_entry, x_offset, BR_x, targety) => {
			...
			if (source_entry.y.length > 0) {
				//underground doesn't have this
				if (source_entry.leftleg_down.x >= BR_x) {
					//ascend
					leftleg_entry = find_lower(
						mountain[source_entry.leftleg_down.x + x_offset],
						newentry.y,
					);
				} else {
					//not ascend
		...
		omega_Y_limit = (seq, FSterm) => {
			...
			for (var n = 1; n <= FSterm; ++n) {
				var ref = top.map((topentry) =>
					find_lower(mountain[mountain.length - 1], topentry.y),
				);
				for (var dx = 1; dx <= width; ++dx) {
					...
					magma_entries[dx].forEach((magma_entry) => {
						...
						while (
							!(
								...
								magma_entries[dx].includes(source_entry.rightleg_up)
							)
						) {
							targety = vertical_increase(
								targety,
								dimension_difference(source_entry.y, source_entry.rightleg_up.y),
							);
					...
					column[0].value = 1;
					column
						.slice(1, column.length - 1)
						.forEach(
							(entry) =>
								(entry.value =
									entry.rightleg_up.value + entry.rightleg_up.leftleg_down.value),
						);
				}
			}
