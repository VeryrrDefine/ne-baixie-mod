...
register.push({
	...
	FS: (() => {
		var data = {},
			...
			extract = (term, index) =>
				index.length ? extract(term[index[0]], index.slice(1)) : term,
			subterm_index = (a) => {
			...
			BuiltQ = (a, b, rc, n) => {
				...
				var extractparent = (x) => (x.length ? extract(a, x.slice(0, x.length - 1)) : b),
					refresh_totest = (d, e) => {
						if (
							typeof extract(a, d) === 'number' ||
							TON_compare(extract(a, d), extractparent(e)) < 0
						)
							return;
						totest.push(d);
				...
				return subterm_index(a).every((x) => {
					...
					if (x.some((t, zindex) => TON_compare(extract(a, x.slice(0, zindex)), b) < 0))
						return true;
					totest = [];
					refresh_totest(x, x);
						...
						if (
							x
								...
								.every(
									(t, dz) =>
										TON_compare(
											extract(a, x.slice(0, y.length + dz)),
											extractparent(y),
										) >= 0,
								) &&
							totest.every(
								(z) => TON_compare(extract(a, z), extractparent(y)) >= 0,
							) &&
							(TON_compare(r(extract(a, y), extractparent(y)), rc) < 0 ||
								BuiltQ(extract(a, y), extractparent(y), rc, n - 1))
						)
							return true;
					}
					return false;
				...
				mainloop: while (true) {
					if (flag) {
						if (typeof beta === 'number' && beta >= 0) {
							...
							//m > 2, the main part of step 1 and 2
							c3 = beta;
							c1 = beta[1][1];
