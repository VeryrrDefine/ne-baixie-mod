(() => {
	 ...
	 */
	var toShort = (expr) =>
			expr.slice(1).map((row) => row.slice(1, -row[0]).concat(row[row.length - 1])),
		...
		copy = (raw, flag) => {
			...
			expr = expr.concat(
				raw
					.slice(active[1 + active[0]], active[1 + active[0]] + flag)
					.map((row) => ap(row, active)),
			);
			for (
				row_idx = active[1 + active[0]];
				row_idx < active[1 + active[0]] + flag;
				++row_idx
			) {
				expr[0].push(
		...
		comp = (raw, i, T) => {
			var expr = raw.slice(0, i).map((row) => row.slice()),
				...
				ci =
					raw[i].length < raw[i][0] * 2 + 1
						...
						: raw[i].slice(1);
			for (var r = 0; r < u; ++r) {
				var values = ci.concat(T.slice(0, 1 + r)).concat(
					Array(r)
						.fill(0)
			...
			for (var ii = i; ii < raw.length; ++ii) {
				...
				var flag =
					isAncestor(raw[0], i, ii) && values.findIndex((x) => x <= i) <= raw[ii][0];
				if (flag) {
		...
		init: () => [
			{
				...
				low: [[[]]],
				subitems: [],
				anal: ['unpleasant embedding notation'],
			},
