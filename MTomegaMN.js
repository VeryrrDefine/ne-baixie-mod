(() => {
	var entry_compare = (a, b) => {
			//each entry = [value,separator] where separator is mountain
			if (a[0] < b[0]) return -1;
			if (a[0] > b[0]) return 1;
		...
		separator_compare = mountain_compare,
		...
		subtract1 = (A0, V, BRij) => {
			...
			if (!BRij) {
				if (mountain_is_one(topright_separator)) {
		...
		extend = (A0) => {
			...
			if (mountain_is_one(topright_separator)) {
				...
				do {
					...
					separator_compare(A0[working[0]][working[1]]?.[1] ?? [], topright_separator) >=
					0
				);
				BRij = [working[0], working[1] + 1];
			}
		...
		threshold = (A, shorter, low, high) => {
			...
			while (true) {
				...
				if (
					vertical_compare(vertical_increase(low, res), vertical_increase(high, res)) >= 0
				)
					return n;
				n++;
			}
