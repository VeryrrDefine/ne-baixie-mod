;(()=>{
   ...
   ,subtract1 = (A0,V,BRij)=>{
      ...
      if(!BRij){
         if(mountain_is_one(topright_separator)){
            ...
            do{
               ...
               --working[1]
            }while(separator_compare(A[working[0]][working[1]]?.[1]??[],topright_separator)>=0);
      ...
      if(mountain_is_one(topright_separator)) A[rightmost].pop()
         ...
         if(vertical_compare(vertical_increase(V[steped[0]][steped[1]-1]??[],topright_separator),V[rightmost][topmost-1]??[])<=0)
            A[rightmost].pop()
         else
   ...
   ,extend = (A0)=>{
      ...
      if(mountain_is_one(topright_separator)){
         ...
         do{
            ...
            --working[1]
         }while(separator_compare(A0[working[0]][working[1]]?.[1]??[],topright_separator)>=0);
         BRij = [working[0],working[1]+1]
      }
   ...
   ,expand = (A0,FSterm,shorter=false)=>{
      ...
      if(mountain_is_limit(topright_separator)){
         ...
         A[rightmost][topmost][1] = expand(topright_separator,
            threshold(topright_separator,shorter,V[steped[0]][steped[1]-1]??[],V[rightmost][topmost-1]??[])+FSterm
         ,shorter)
         return A
      }
