(() => {
	var entry_compare = (a, b) => {
			//each entry = [value,separator] where separator is number
			if (a[0] < b[0]) return -1;
			if (a[0] > b[0]) return 1;
			return separator_compare(a[1], b[1]);
		},
		column_compare = (a, b) => {
			//each column = [entry,entry,...,entry]
			var i = 0,
				c;
			while (true) {
				if (i >= a.length) {
		...
		mountain_compare = (a, b) => {
			//each mountain = [column,column,...,column]
			var i = 0,
				c;
			while (true) {
				if (i >= a.length) {
		...
		separator_compare = (a, b) => Math.sign(a - b),
		mountain_is_limit = (m) => m.length > 0 && m[m.length - 1].length > 0,
		mountain_display = (m) =>
			m
				.map((column) => '(' + column.map(([v, sep]) => ','.repeat(sep) + v).join('') + ')')
				.join(''),
		vertical_compare = (a, b) => {
			//each vertical = [separator,separator,...,separator]
			var i = 0,
				c;
			while (true) {
				if (i >= a.length) {
		...
		vertical_increase = (v, m) => {
			var i = v.length - 1;
			while (i >= 0 && separator_compare(v[i], m) < 0) --i;
		...
		subtract1 = (A0, V, BRij) => {
			...
			var steped = Parent(A, V, [rightmost, topmost]);
			if (1 === topright_separator) A[rightmost].pop();
			else {
				topright_separator = topright_separator - 1;
				if (
					vertical_compare(
						vertical_increase(V[steped[0]][steped[1] - 1] ?? [], topright_separator),
						V[rightmost][topmost - 1] ?? [],
					) <= 0
				)
					A[rightmost].pop();
				else A[rightmost][topmost][1] = topright_separator;
			}
		...
		extend = (A0) => {
			var rightmost = A0.length - 1;
			var topmost = A0[rightmost].length - 1;
			var topright_separator = A0[rightmost][topmost][1];
			...
			if (1 === topright_separator) {
				...
				do {
					...
					separator_compare(A0[working[0]][working[1]]?.[1] ?? 0, topright_separator) >= 0
				);
				BRij = [working[0], working[1] + 1];
			}
				...
				A0[x].forEach((entry, y) => {
					...
					if (~source_magmas[y]) {
						...
						for (var j = refs[BRindex - 1] + 1; j <= refs[BRindex]; ++j) {
							if (j === refs[BRindex]) target_column.push([value + width, entry[1]]);
							else target_column.push([value + width, A[BRij[0] + width][j][1]]);
						}
					} else {
						target_column.push([value + (value > BRij[0] ? width : 0), entry[1]]);
					}
				});
			}
			return A;
