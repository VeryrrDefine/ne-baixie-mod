...
function cOCF_display(x) {
	...
	if (c == 'p(0)' && d == '0') {
		...
		let k = cOCF_exp(
			cOCF_mul('P(0)', cOCF_div(cOCF_log(cOCF_lastTerm(cOCF_arg(x))[1]), 'P(0)')),
		);
...
function cOCF_div(a, b) {
	...
	return cOCF_add(
		cOCF_exp(cOCF_sub(cOCF_log(a), cOCF_log(b))),
		cOCF_div(cOCF_firstTerm(a)[1], b),
	);
}
...
function cOCF_mul(a, b) {
	...
	return cOCF_add(
		cOCF_exp(cOCF_add(cOCF_log(a), cOCF_log(b))),
		cOCF_mul(a, cOCF_firstTerm(b)[1]),
	);
}
...
register.push({
	...
	init: () => [
		{ expr: Infinity, low: ['0'], subitems: [], anal: ['c'] },
