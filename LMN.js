...
;var LMN_display = x=>''+x==='true,Infinity'?'Limit':
   x===0?'0':x[0]?'ψ<sub>'+x[1]+'</sub>('+LMN_display(x[2])+')':x.slice(1).map(LMN_display).join('+')
,LMN_compare = (x,y)=>{
   if(x===0){
      return y===0?0:-1
   }
...
register.push({
   ...
   ,FS:(()=>{
      var data={}
      ,Copy = x=>typeof x==='number'?x:[x[0]].concat(x.slice(1).map(Copy))
      ,maxsummand = x=>{
         if(!x||x[0]) return x
      ...
      ,LMN_FS = (x,FSterm)=>{
         ...
         ,s=termtier(xi)
         if(s===j-1){
            if(LMN_compare(xi,change(xk,xk))>=0){//4.3.4.3 of LMN
               prev=x2
      ...
      return (x,FSterm)=>{
         ...
         return data[datakey][FSterm] = LMN_FS(x,FSterm)
      }
   })()
   ,init:()=>([
