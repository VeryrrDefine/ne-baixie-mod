			...
			if (s.indexOf('v') == -1 || !isFinite(Number(s.substring(s.indexOf('v') + 1)))) {
				...
				return {
					...
					parentIndex: Math.max(
						Math.min(i - 1, Number(s.substring(s.indexOf('v') + 1))),
						-1,
					),
					forcedParent: true,
				};
				...
				for (var j = mountain.length - 1; j >= 0; j--) {
					...
					while (true) {
						if (height == 0) {
							...
							while (
								mountain[height][m].position <
								mountain[height - 1][l].position - 1
							)
								m++;
							if (
								mountain[height][m].position ==
						...
						if (
							...
							diagonalTree.push(
								(mountain[height][lastIndex]
									? mountain[height][lastIndex].position
									: -1) + height,
							);
							break;
						}
		...
		function expand(s, n, stringify) {
				...
				for (var i = 1; i <= n; i++) {
					...
					for (var j = badRootSeam; j < afterCutLength; j++) {
						...
						if (isAscending) {
								...
								if (k < badRootHeight) {
									...
									result[k].push({
										value:
											parentIndex == -1
												? newDiagonal[0][
														j + (afterCutLength - badRootSeam) * i
													].value
												: NaN,
									...
									k <=
									badRootHeight +
										(cutHeight - badRootHeight) * (i - isReplacingCut)
								) {
									//Br replace
							...
							if (isReplacingCut)
								console.warn(
									'Cut child and not connected to bad root. Makes sense.',
								);
			...
			for (var i = result.length - 1; i >= 0; i--) {
				...
				for (var j = 0; j < result[i].length; j++) {
					...
					result[i][j].value =
						result[i][result[i][j].parentIndex].value + result[i + 1][k].value;
				}
			}
			var rr;
			if (stringify) {
				...
				for (var i = 0; result[0] && i < result[0].length; i++) {
					rr.push(
						result[0][i].value +
							(result[0].forcedParent ? 'v' + result[0].parentIndex : ''),
					);
				}
				rr = rr.join(',');
			} else {
