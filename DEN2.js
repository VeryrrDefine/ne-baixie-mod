(() => {
	...
	var toShort = (expr) =>
		...
		cut = (expr) =>
			expr.slice(0, -1).map((row) => [row[0]].concat(row.slice(1).map((x) => x.slice()))),
		seqFrom = (expr, i, j) => {
			var row = expr[i],
				val = row[j][0],
		...
		copy = (raw, flag) => {
			...
			for (i = begin - 1; i < end - 1; ++i) {
				...
				for (j = 1; j < row.length; ++j) {
					...
					if (
						targetrow[j + targetrow[0] - 1]?.[0] <= active[active.length - 1][0] &&
		...
		compFrom = (raw, r, T) => {
			...
			for (qq = 1; qq <= q; ++qq)
				for (var uu = 2; uu <= 1 + qq; ++uu) expr[r + qq][uu][1] = true;
			var m = (x, idx) => {
				if (!idx) return x;
				var xx = x.slice();
	...
	register.push({
		...
		init: () => [
			{ expr: [Infinity], low: [[]], subitems: [], anal: ["get unpleasant\'nt"] },
