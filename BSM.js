			...
			expand = (m, FSterm) => {
				var parent = (x, y) => {
					...
					ascending = (r, x, y) => {
						...
						return (ascending_cache[str] =
							r <= x && (roots.includes(x) || ascending(r, parent(x, y), y)));
					},
					delta = (r) =>
						m[r].map((value, y) =>
							y < LNZ ? child[y] - value : y === LNZ ? child[y] - value - 1 : 0,
						),
					expansion = (r, n) => {
						var a,
							x,
						...
						for (a = 1; a <= n; ++a) {
							for (x = r; x < endcol; ++x) {
								ss.push(
									ss[x].map(
										(value, y) => value + a * delr[y] * ascending(r, x, y),
									),
								);
							}
						}
					...
					expansionappend = (r) => {
						var delr = delta(r),
							res = expansion(r, 1);
						res.push(
							m[endcol].map((value, y) => value + delr[y] * ascending(r, endcol, y)),
						);
						return res;
					},
					endcol = m.length - 1,
				...
				n = roots.findIndex((r) =>
					specialroots.includes(r)
						...
						: matrix_compare(expansionappend(r), threshould) < 0,
				);
